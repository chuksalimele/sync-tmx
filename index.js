  ...
  window.onerror = function(message, source, lineno, colno, error) {
    ...
    alert("Error occurred: "+ message+ " at "+ source+ " GO TO Line : "+ lineno + " : "+ colno+ " - " + error);
    // You can log the error to a server or take other appropriate action here
...
alertify.defaults.notifier.position = "bottom-right";
  ...
  $(document).ready(function () {
    ...
    $("#btn_trade_inputs").on("click", function () {
      hideCenterContents();
      $("#btn_trade_inputs").addClass("active");
      $("#center_content_trade_inputs").fadeIn();
      displayTradeInputs();
    });
    ...
    $("#ensure_install_uptodate").on("click", function(){
      ipc.send("ensure-install-uptodate");
    });
    ...
    ipc.on("re-started-terminal", function (event, arg) {
      console.log("re-established-pairing", arg);
      var msg = `Successfully re-started terminal to restore it back online after offline detection [${arg.broker}, ${arg.account_number}]`;
      addInfoLog(msg);
      alertify.success(msg, 0);
    });
    
    ipc.on("re-established-pairing", function (event, arg) {
      console.log("re-established-pairing", arg);
      var msg = `Successfully re-established pairing [${arg.broker}, ${arg.account_number}] and [${arg.peer.broker}, ${arg.peer.account_number}]`;
      addInfoLog(msg);
      alertify.success(msg, 0);

    });
    
    ipc.on("re-start-terminal-failed", function (event, arg) {
      console.log("re-start-terminal-failed", arg);      
      var acccount = arg.account;
      var error = arg.error;      
      var msg = `Failed to restart terminal [${acccount.broker}, ${acccount.account_number}] and [${arg.peer.broker}, ${arg.peer.account_number}] - ERROR: ${error}`;
      addErrorLog(msg);
    ...
    ipc.on("sync-exit-at-peer-stoploss-success", function (event, arg) {
      console.log("sync-exit-at-peer-stoploss-success", arg);

      addSuccessLog(
        `[${arg.account.broker}, ${arg.account.account_number}] sync exit at peer stoploss successful. Own ticket is ${arg.own_ticket} and peer ticket is ${arg.peer_ticket}`
      );
    ...
    ipc.on("sync-exit-at-peer-stoploss-fail", function (event, arg) {
      console.log("sync-exit-at-peer-stoploss-fail", arg);

      addErrorLog(
        `[${arg.account.broker}, ${arg.account.account_number}] sync exit at peer stoploss failed! ${arg.last_error}. Own ticket is ${arg.own_ticket} and peer ticket is ${arg.peer_ticket}`
      );
    ...
    ipc.on("sync-partial_close-success", function (event, arg) {
      console.log("sync-partial_close-success", arg);
      var account = arg.account;

      addSuccessLog(
        `[${account.broker}, ${account.account_number}] sync partial close successful.`
      );      
    ...
    ipc.on("sync-partial_close-fail", function (event, arg) {
      console.log("sync-partial_close-fail", arg);
      var account = arg.account;

      addErrorLog(
        `[${account.broker}, ${account.account_number}] sync partial close failed! ${account.last_error}`
      );
    ...
    ipc.on("trade-properties-save-success", function (event, arg) {
      console.log("trade-properties-save-success", arg);

      mergeObjectTo(arg, AppConfig);

      displayTradeProperties(true);
    });

    ipc.on("trade-properties-save-fail", function (event, arg) {
      console.log("trade-properties-save-fail", arg);

      displayTradeProperties(false);
    });
...
function hideCenterContents() {
  ...
  $("#center_content_trade_inputs").hide();  
  ...
  $("#btn_trade_inputs").removeClass("active");
...
function displayTradeProperties() {
  $(".trade_properties_saved").show();
  $(".trade_properties_edit").hide();
  
  var prop = AppConfig["trade_properties"];

  if(prop){
    document.getElementById("trade_properties_sync_copy_manual_entry").checked = prop["sync_copy_manual_entry"] == "true"
    document.getElementById("trade_properties_only_trade_with_credit").checked = prop["only_trade_with_credit"] == "true"
    document.getElementById("trade_properties_enable_exit_at_peer_stoploss").checked = prop["enable_exit_at_peer_stoploss"] == "true"
    document.getElementById("trade_properties_exit_clearance_factor").value = prop["exit_clearance_factor"]
  }


    document.
    getElementById("trade_properties_sync_copy_manual_entry").checked?
    $('#lbl_trade_properties_sync_copy_manual_entry').addClass('checkmark icon'):
    $('#lbl_trade_properties_sync_copy_manual_entry').removeClass('checkmark icon');
  
    document.
    getElementById("trade_properties_only_trade_with_credit").checked?
    $('#lbl_trade_properties_only_trade_with_credit').addClass('checkmark icon'):
    $('#lbl_trade_properties_only_trade_with_credit').removeClass('checkmark icon');
  
    document.
    getElementById("trade_properties_enable_exit_at_peer_stoploss").checked?
    $('#lbl_trade_properties_enable_exit_at_peer_stoploss').addClass('checkmark icon'):
    $('#lbl_trade_properties_enable_exit_at_peer_stoploss').removeClass('checkmark icon');
     

    document.getElementById("lbl_trade_properties_exit_clearance_factor").innerHTML = document.getElementById("trade_properties_exit_clearance_factor").value + "%";
...
function SaveTradeProperties(){

  var trade_properties = {};

   var trade_properties_paired = document.getElementById("trade_properties_paired").value;

   trade_properties.sync_copy_manual_entry = document.getElementById("trade_properties_sync_copy_manual_entry").checked + "";
   trade_properties.only_trade_with_credit = document.getElementById("trade_properties_only_trade_with_credit").checked + "";
   trade_properties.enable_exit_at_peer_stoploss = document.getElementById("trade_properties_enable_exit_at_peer_stoploss").checked + "";
   trade_properties.exit_clearance_factor = document.getElementById("trade_properties_exit_clearance_factor").value;

  var obj = {}
  
  if(trade_properties_paired == "default"){
    obj.account_list = getAllAccounts();    
  }else{
  ...
  //modify
  app_config["trade_properties"] = trade_properties;

  
  obj.config = app_config;

  ipc.send("save-trade-properties", obj);

}

function EditTradeProperties(){

  $(".trade_properties_saved").hide();
  $(".trade_properties_edit").show();
...
function symbolsConfigurationHTML(edit, add, saved) {
  ...
  for (var k in accounts) {
    td += `<td  class="definition" colspan='1'>${accounts[k].broker}<br/>${accounts[k].account_number}</td>`;
  }

  var head_row = `<tr>
                         <td rowspan="2">Symbol</td>
                         <td  class="definition" rowspan="2">Spread</td>   
                         <td class="definition" colspan="${accounts.length}">Brokers Relative Symbols</td>
                    </tr>
                    <tr>
                        ${td}
                    </tr>`;
...
function pairedAccountHTML(pair) {
  ...
  var table = `
    ...
    accountB
                  ...
                  <tr>
                    <th colspan="2">
                    <div style="font-style:italic; margin-bottom: 20px; font-style:italic;">
                       Profit Range
                    </div>
                    <div>
                       Balance Range
                    ...
                    <th class="green">
                      <div style="color:#222; margin-bottom: 20px;">
                        ${accountA.expected_exit_profit.toFixed(2)} ${accountA.account_currency + '  /  '}${accountA.expected_target_profit.toFixed(2)} ${accountA.account_currency}
                      </div>
                      <div style="color:teal;">
                        ${accountA.expected_exit_balance.toFixed(2)} ${accountA.account_currency + '  /  '}${accountA.expected_target_balance.toFixed(2)} ${accountA.account_currency}
                      </div>
                    </th>
                    <th class="green">
                      <div style="color:#222; margin-bottom: 20px;">
                        ${accountB.expected_exit_profit.toFixed(2)} ${accountB.account_currency + '  /  '}${accountB.expected_target_profit.toFixed(2)} ${accountB.account_currency}
                      </div>
                      <div style="color:teal;">
                        ${accountB.expected_exit_balance.toFixed(2)} ${accountB.account_currency + '  /  '}${accountB.expected_target_balance.toFixed(2)} ${accountB.account_currency}
                      </div>
                    </th>
                  </tr>
