...
class Emailer {
    ...
    CheckToNotifySessionInformationAtInterval(account, notify_session_information_every_interval_in_seconds) {
        if (!account.Peer()) {
            return;
        }
    ...
    CheckToNotifyNearStopout(account, notify_at_percent_close_to_stopout_input) {
    ...
    CheckToNotifyMarginCall(account) {
        if (account.AccountEquity() >= account.AccountBalance()) {
    ...
    OrderOpenNotify(account, order) {
        ...
        SyncUtil_1.SyncUtil.AsyncWaitWhile(this.send.bind(this, subject, body), () => order.stoploss == 0 || order.target == 0 || peer_order.stoploss == 0 || peer_order.target == 0);
    }
    OrderCloseNotify(account, order) {
    ...
    verifyConnection(param, callback) {
